...
export async function fetchStates(countryId) {
  try {
    ...
    console.log('Fetching states for country ID:', numericCountryId)
    
    const response = await axios.get(`${API_BASE_URL}/states/${numericCountryId}`)
    const data = response.data
...
export async function fetchCities(stateId) {
  try {
    ...
    console.log('Fetching cities for state ID:', numericStateId)
    console.log('API URL:', `${API_BASE_URL}/cities/${numericStateId}`)
    
    const response = await axios.get(`${API_BASE_URL}/cities/${numericStateId}`)
    const data = response.data
